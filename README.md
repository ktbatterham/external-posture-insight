...
- A consent-safe interactive continuation from a successful local summary scan into
  ...
  runs an authoritative hosted scan, and lets the user choose a daily or weekly watch.
  The prompt states that the target URL is sent to SecURL, defaults to No, and never appears
  in CI, pipes, files, structured output, or policy runs.
