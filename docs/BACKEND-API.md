...
Header values are assertions, not authentication. Telemetry records their provenance and exposes separate `verified`, `unverified`, and `automation` partitions under `clients.identity.attribution`. An assertion is verified only when the request has already passed session, API-key, or owner-token authorization, or when the product identity is inferred by a fixed server endpoint. Unauthenticated header claims remain `unverified` and cannot increase trusted adoption counts. Client ids or versions that indicate smoke, CI, DAST, or other automation are forced into the `automation` partition even if they accompany authenticated requests or claim a release channel. Existing aggregate identity fields remain available for compatibility and continue to include all accepted bounded assertions; integrity-sensitive reporting should use the new attribution partitions.
...
`GET /api/scans/:id/digest` returns a compact posture digest for lightweight clients, including evidence quality and signal clarity so clients can show how trustworthy the scan is, why the grade landed where it did, and what to do next. `GET /api/scans/:id/insights` returns a display-ready posture intelligence layer: risk themes, top insights, and next-best actions derived from the action plan without requiring clients to interpret raw findings. `GET /api/scans/:id/mobile-summary` combines the scan summary, digest, and insights into one compact app result payload so mobile clients can avoid loading the full scan object after scan completion. `GET /api/scans/:id/share-card` returns public, lightweight report-sharing metadata: title, summary, grade, score, top issues, score drivers, next best action, native share text, and web links. `GET /api/scans/:id/observations` returns Observation Ledger v1: deterministic, source-aware evidence records with explicit status, confidence, observation time, and freshness. `GET /api/scans/:id/observation-drift` compares those stable records with the previous completed scan and classifies regressions, improvements, and neutral changes without treating ordinary certificate-day decay as an alert. `GET /api/scans/:id/manifest` returns Posture Manifest v1: a machine-readable external security recipe card containing the target, scan mode/timing, score and score drivers, observation ledger, skipped assessment context, evidence quality, signal clarity, engine version, and resolved policy evaluation. `GET /api/scans/:id/brief` returns a concise exposure brief for mobile, CLI, and report clients that need the highest-priority public entry points, trust gaps, abuse indicators, and next actions without loading the full evidence payload. `GET /api/scans/:id/vendors` returns a compact vendor and supply-chain exposure brief covering third-party providers, visible data-flow categories, SRI gaps, priority vendors, and next actions. `GET /api/scans/:id/action-plan` returns a prioritized fix narrative that combines remediation, score drivers, exposure brief, and vendor context into owner/effort/impact-ranked actions. `GET /api/scans/:id/drift` returns the existing high-level drift/risk-event analysis. Export resources return machine-readable JSON, Markdown, SARIF, or CI JSON once the scan is complete.

From `securl@1.23.0`, the additive `/vendors` payload is External Exposure Inventory v1. It retains the existing vendor summary and adds stable IDs across visible third-party, infrastructure, identity, and AI providers, with role, data-flow, evidence, confidence, SRI integrity status, review priority, and drift-friendly inventory counts. These signals describe passive public evidence and do not claim knowledge of internal dependency or data-flow configuration.
...
Posture monitoring targets may include an optional object-shaped `policy` when created or updated through `POST /api/monitoring-targets`. Policies are limited to 25 declarative rules and support exact kind, kind-prefix, or category selectors with `eq`, `neq`, `in`, `gte`, and `lte` assertions. Rules can evaluate current observations or observation changes. Invalid or executable policy content is rejected. Targets without a custom policy use `securl-baseline-v1`. `GET /api/scans/:id/policy-evaluation` resolves the matching target policy when available and otherwise evaluates the maintained baseline. `GET /api/scans/:id/manifest` embeds that same resolved policy evaluation so CI, mobile, and reporting clients can consume one audit-friendly artifact.

Certificate monitoring targets may include a string-shaped `policy` when `kind:"cert"`: `production`, `strict`, or `renewal-watch`. The profile is stored per target, echoed as `target.policy` and `target.cert.policyProfile`, and drives server-side expiry warning thresholds. Targets without a certificate policy keep the legacy 30/14/7/1-day warning bands. `production` starts expiry warning at 14 days; `strict` and `renewal-watch` start at 30 days.
...
The smoke command checks health, readiness, capabilities, scan creation, scan detail resources, digest/brief/vendors/action-plan/events/evidence/observations/manifest resources, live certificate lookup, comparison/drift resources, export formats, and the public share resource.
...
When `GET /api/capabilities` advertises `mobile-monitoring-explanations-v1`, each monitoring event on the mobile summary and target-detail comparison also includes `targetId`, `eventId`, and `deepLink: { route: "monitoring_target", targetId, eventId }`. Drift and certificate pushes expose the same three top-level fields. Clients should route by `targetId`, optionally focus `eventId`, and continue falling back to the existing `host`/`url` fields for older payloads.
...
`GET /api/monitoring-cert-summary` is a Cert Watch-optimized owner-scoped summary. It returns only certificate monitoring targets, aggregate counts for healthy, expiring, expired, unreachable, and unknown certificates, the next scheduled check, recent certificate changes, monitoring events, active certificate policy profiles, and APNs health for active `com.ktbatterham.certwatch` device registrations. The endpoint also records privacy-safe active-client telemetry keyed by a hashed owner/scope value, so repeated watch-list refreshes can be separated from genuinely distinct Cert Watch users without storing device identifiers.
